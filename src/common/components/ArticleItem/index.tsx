 ...
 */
import { addToBookmarks, removeFromBookmarks } from "@Store/reducers/bookmarks/actions";
import { toKebabCase } from "@Utils/helpers";
import { getBookmarksPosts } from "@Store/selectors/bookmarks";

function ArticleItem({ article }) {
  const dispatch = useDispatch();
  const bookmarksPosts = useSelector(getBookmarksPosts);
  ...
  return (
    <article style={{ backgroundImage: `url(${article.image})` }}>
      <a href={article.url} target="_blank" rel="noreferrer" className="article__overlay-link">
        &nbsp;
      </a>
      <Link href={"/posts/source/[slug]"} as={`/posts/source/${toKebabCase(article.source)}`}>
        <a className="research__related-label">{article.source}</a>
      </Link>
      {isLatestResearch && <span className="research__latest-label">Latest research</span>}
      <h2 className="article__title">{article.headline}</h2>
      <p className="article__summary">{cutSummaryText(article.summary)}</p>
      ...
      <button
        ...
        title={isAddedToBookmarks ? "Remove from Bookmarks" : "Add to Bookmarks"}
